document.addEventListener('DOMContentLoaded', function() {
    ...
    var loop = setInterval(function() {
        if (minute < 10){
            document.getElementById("timer").innerHTML = timeLeft+":0"+minute;
        }
        else {
            document.getElementById("timer").innerHTML = timeLeft+":"+minute;
        }
        minute = minute-1;
        if (minute == -1) {
            minute = 59;
            timeLeft = timeLeft-1;
            if (timeLeft == -1) {
                window.location.href = '../Tests_And_Grades/tests_grades.html'; //Redirects if time is up
            }
        }
    }, 1000)
    ...
    function loadTest() {
        ...
        document.getElementById("timer").innerHTML = test.timeAllocation+":00";
        timeLeft = test.timeAllocation-1;
        renderQuestions();
        updateTotalPoints();
    }
    ...
    function createChoice(question, index) {
        ...
        input.className = 'choice-select'; //aaaa
        input.checked = false; //question.correctAnswers.includes(index);
        ...
        textInput.type = 'text';
        textInput.className = 'question-gap';
        textInput.text = question.choices[index] || '';
    ...
    document.querySelector('.finish-button').addEventListener('click', () => {
        window.location.href = 'tests_grades.html';
    });
